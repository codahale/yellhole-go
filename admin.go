...
func handleNewNote(config *config, queries *db.Queries, templates *templateSet) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		http.Redirect(w, r, config.BaseURL.JoinPath("note", id).String(), http.StatusSeeOther)
	})
}
